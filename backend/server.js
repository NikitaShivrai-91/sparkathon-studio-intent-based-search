...
app.get('/health', (req, res) => {
  res.json({
    status: 'healthy',
    service: 'Bedrock DataMaps Search API',
...
app.get('/api/datamaps', (req, res) => {
  ...
  // busNo filtering only if the field exists
...
app.post('/api/datamaps/explain/:dataMapId', async (req, res) => {
  try {
    ...
    // Try to find by dataMapId first, then by dataMapName
    ...
    console.log(`\nExplaining DataMap: ${dataMap.dataMapName}`);
...
app.listen(port, () => {
  console.log(`
╔══════════════════════════════════════════════════════════╗
║   Bedrock DataMaps Search API Server                    ║
║                                                           ║
║   🚀 Server running on: http://localhost:${port}         ║
║   🤖 AI Model: Claude Sonnet 4.5 (AWS Bedrock)           ║
║   📊 Loaded DataMaps: ${dataMaps.length}                            ║
║                                                           ║
║   Available Endpoints:                                    ║
║   GET  /health                                            ║
║   GET  /api/datamaps                                      ║
║   POST /api/datamaps/search                               ║
║   POST /api/datamaps/explain/:dataMapId                   ║
║   GET  /api/datamaps/:dataMapId                           ║
╚══════════════════════════════════════════════════════════╝
  `);
});
